...
class Block:
    def __init__(self, transactions, difficulty, version,  hash=None, nonce=None, previousHash=None, merkleRoot=None,
                 timestamp=datetime.now().strftime('%d/%m/%Y,%H:%M:%S'), transactionsNumber=0, size=0):
        ...
        if isPow2(len(transactions)) and len(transactions) <= 512:
            ...
            self.transactionsNumber = transactionsNumber  # int 2
            ...
            if transactionsNumber == 0:
                self.transactionsNumber = len(transactions)
            ...
            if nonce is None:
                nonce, digest = miner.mine(self.timeStamp, self.merkleRoot, self.transactionsNumber, self.size,
                                           self.difficulty)
                self.nonce = nonce
                self.hash = digest
    ...
    def toString(self):
        return 'timestamp: {} hash: {} nonce: {} root: {} transactions: {} size: {}'.format(
            self.timeStamp, self.hash, self.nonce, self.merkleRoot, self.transactionsNumber, self.size)
    ...
    def toJson(self):
        return {
               ...
               'nonce': self.nonce,
               'transactionsNumber': self.transactionsNumber,
               'hash': self.hash,
